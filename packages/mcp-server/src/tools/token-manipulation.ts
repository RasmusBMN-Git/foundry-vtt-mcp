  ...
  getToolDefinitions() {
    return [
      ...
      },
      {
        ...
        description:
          "Fire an NPC's attack or spell through its attack/save Activity (no GM dialog). For an ATTACK, this fast-forwards the d20 attack roll AND, on a hit (attack total vs each target's AC; a natural 20 always hits + crits, a natural 1 always misses), auto-rolls damage and applies it to that target through the same gate as apply-damage: an NPC target takes it automatically, a PC target returns needs_approval (D4) unless trustedMode — a miss applies nothing. For a SAVE spell that deals damage (e.g. Sacred Flame, Fireball), each NPC target now auto-resolves NPC-vs-NPC (T37): the target rolls its own save vs the spell DC, spell damage is rolled once and shared across targets, and it is applied through the same gate — FULL on a failed save, and on a made save the spell's on-save amount (HALF via the native damage multiplier, so resistances still apply, or NONE for cantrips like Sacred Flame). The per-target outcome (attack or save) is in the response's `results` array; a save entry carries { tokenId, tokenName, saveTotal, dc, saveSucceeded, onSave, multiplier, damage, damageParts, decision, hpBefore, hpAfter } (decision is auto / save_no_damage / needs_approval / invalid_target / no_save_actor). For a HEALING spell (e.g. Cure Wounds, Healing Word) that carries a heal Activity, this rolls the healing once and raises each listed target's HP through the same gate as apply-damage (as a type:\"healing\" write, clamped at max HP): an NPC target heals automatically, a PC target returns needs_approval (D4) unless trustedMode — a heal is still a write to HP the DM owns, and its per-target result carries { tokenId, tokenName, healing, healingParts, decision, hpBefore, hpAfter } with activityType:'heal'. (Pure buff/utility spells that only apply an ActiveEffect — Bless, Shield of Faith — are NOT covered here.) This is NOT use-item: use-item forces a configuration dialog and returns requiresGMInteraction, this tool does not. NPC-only as the actor: a PC token as the acting actor is rejected outright regardless of trustedMode (D2 — Claude never fires the PC's own items). A PC target of a SAVE spell is NEVER auto-rolled or auto-damaged: this tool posts a roll request to the player via request-player-rolls (D2, reported in pcSaveRequested / the target's pc_save_requested result entry) and the DM applies the consequence out of band. Status CONDITIONS and a save spell's on-FAIL EFFECTS (e.g. Hold Person paralysis) are still NOT auto-applied — follow up with toggle-token-condition under its own gate.",
        inputSchema: {
          type: 'object',
          properties: {
            tokenId: {
